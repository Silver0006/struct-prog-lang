...
def verify_same_tokens(a, b):
    return clean(eval(tokenize(a))) == clean(eval(tokenize(b)))


def test_multiple_tokens():
    print("testing multiple tokens...")
    assert clean(eval(tokenize("1+2"))) == [
        {"tag": "number", "value": 1},
        {"tag": "+"},
        {"tag": "number", "value": 2},
        {"tag": None},
    ]
    assert clean(eval(tokenize("1+2-3"))) == [
        {"tag": "number", "value": 1},
        {"tag": "+"},
        {"tag": "number", "value": 2},
        {"tag": "-"},
    ...
    assert clean(eval(tokenize("3+4*(5-2)"))) == [
        {"tag": "number", "value": 3},
        {"tag": "+"},
        {"tag": "number", "value": 4},
        {"tag": "*"},
...
def test_if_identifier_sequence():
    ...
    t = clean(eval(tokenize("if alpha beta")))
...
def test_line_and_column_tracking():
    ...
    tokens = eval(tokenize("42"))
    assert tokens[0]["line"] == 1
    assert tokens[0]["column"] == 1

    # Single line, multiple tokens
    tokens = eval(tokenize("1 + 2"))
...
def test_strings_with_line_column():
    ...
    # Simple string
    tokens = eval(tokenize('"hello"'))
    assert tokens[0]["line"] == 1 and tokens[0]["column"] == 1

    # String with spaces
    tokens = eval(tokenize('x = "hello world"'))
    assert tokens[0]["line"] == 1 and tokens[0]["column"] == 1  # "x"
    assert tokens[1]["line"] == 1 and tokens[1]["column"] == 3  # "="
...
def test_comments_with_line_tracking():
    ...
    tokens = eval(tokenize("x = 1 // comment\ny = 2"))
